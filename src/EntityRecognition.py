...
class EntityRecognition:
    ...
    def extract(self):
        def process(row):
            ...
            def spacy_extract(text, p_id):
                document = spacy_nlp(text)
                entity_list = []
                for element in document.ents:
                    text = element.text.strip("\n").replace("\n", "").replace("\r", "")
                    if element.label_ not in ["CARDINAL", "DATE", "QUANTITY", "TIME", "ORDINAL", "MONEY", "PERCENT", "QUANTITY"]:
                        entity = dict(type=element.label_, text=text, p_id=p_id)
                        entity_list.append(entity)
                return entity_list
            entities = [spacy_extract(sentence, p_id) for p_id, sentence in enumerate(row["sentences"])]
            return {"_id": row["_id"], "sentences_entities": entities, "sentences": row["sentences"] }
    ...
    def join_sentences(self):
        def execute_join(row):
            ...
            return {"_id": row["_id"], "entities": entities, "sentences": row["sentences"] }
